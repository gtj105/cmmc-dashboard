...
- Secrets (DB password, NextAuth secret) are stored in `./secrets/` — never committed to git
- Passwords are bcrypt-hashed (10 rounds); minimum 12 characters enforced
- CSRF double-submit cookie protection on all mutation endpoints
- JWT sessions with 24-hour expiry; tokens are revoked server-side on logout
- Full Content Security Policy, rate limiting (10 req/s API, 3 req/s login), structured audit log
- Zod input validation on all API routes — oversized or malformed payloads are rejected before hitting the DB
- Backup exports do **not** contain password hashes — after a restore, all users must reset their passwords
...
- **PostgreSQL 15** — practices, POA&M, evidence metadata, audit log, session revocation
