...
echo "${BACKUP_SCHEDULE} . ${ENV_FILE} && /usr/local/bin/backup.sh >> /var/log/backup.log 2>&1" > "${CRON_FILE}"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Cron daemon starting. Backups scheduled at: ${BACKUP_SCHEDULE}"
